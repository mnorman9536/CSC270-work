  ...
  def makePages( cexFilePath: String ): Unit = {
  	...
  	try {
		  	...
		  	println(s"\n--------\nconstructing website\n--------\n")
				...
				val corpVec: Vector[Corpus] = hocuspocus.corpusToChapters( corp, drop = 1 ) //breaks text up into chapter sized blocks
				//val corpVec: Vector[Corpus] = hocuspocus.equalDivs( corp, n = 10 ) //how many citable page, gives as close to that number as possible
				//val corpVec: Vector[Corpus] = hocuspocus.equalSize( corp, target = 5000 ) //like a printed book, each page has a similar amount of information
				/* -------------------------------------- */
